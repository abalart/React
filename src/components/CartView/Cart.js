...
import { Context} from "../../Context/CustomProvider";  //Context=CartContext

export const Cart = () => {
    
    const { cart, total, deleteProduct, resetCart } = useContext(Context); //Se comparten las variables por contexto

     

    if(cart.leght === 0){
        return (<h1> No agregaste productos,puede ver 
            <Link to= '/'> aca </Link></h1>)
    }

    //Simulando info de un comprador
    const comprador = {
    nombre: "Juan",
    apellido: "Perez",
  ...
  const finalizarCompra = ()=>{
    const ventasCollection = collection(db,"sales"); //collection ventas
